      ...
      >
        {recommendations.map((rec) => (
          <SwiperSlide key={rec.id} className="pb-4">
            {/* 💡 수정됨: 
                1. aspect-[16/10] -> aspect-[4/3] (세로 높이 증가)
                2. rounded-2xl -> rounded-xl (라운드 값 감소) 
            */}
            <Link 
              href={`/projects/${rec.id}`} 
              replace 
              className="group/item relative block overflow-hidden rounded-xl border border-white/10 bg-[#2a3437] aspect-[4/3]"
            >
              <Image
